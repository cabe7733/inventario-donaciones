...
export function ContextHelp() {
  ...
  return <><button type="button" aria-label={`Ayuda: ${help.title}`} onClick={() => setOpen(true)} className="flex h-10 w-10 items-center justify-center rounded-lg text-text-secondary hover:bg-neutral-100 hover:text-fg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-400"><Question size={20} /></button><Modal open={open} onClose={() => setOpen(false)} title={help.title}><div className="flex flex-col gap-4"><p className="text-body text-text-secondary">{help.body}</p><ol className="list-decimal space-y-2 pl-5 text-body-sm">{help.steps.map((step) => <li key={step}>{step}</li>)}</ol></div></Modal></>;
}
