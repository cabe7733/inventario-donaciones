...
import { fetchProducts, createProduct, type Product } from '../../lib/db';
...
export function MovimientoPage() {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      <header>
        <h1 className="text-h2">{kind === 'entrada' ? t('movimientos.entrada') : t('movimientos.salida')}</h1>
      </header>
      <Segmented<MovementKind> ariaLabel={t('movimientos.kind.select')} value={kind} onChange={setKind} options={[{ value: 'entrada', label: t('movimientos.entrada'), icon: <ArrowDownRight size={18} aria-hidden="true" /> }, { value: 'salida', label: t('movimientos.salida'), icon: <ArrowUpRight size={18} aria-hidden="true" /> }]} />
      <AutocompleteOrCreate id="m-product" label={t('movimientos.producto')} required value={productId} onChange={setProductId} items={productItems} onCreate={createNewProduct} error={error} hint={selected ? t('movimientos.stockDisponible', { stock: String(selected.total_stock) }) : undefined} />
      <Field id="m-qty" label={t('movimientos.cantidad')} required>
        <input id="m-qty" type="text" inputMode="numeric" pattern="[0-9]*" autoComplete="off" className={inputClass} value={qty} onChange={(e) => setQty(e.target.value.replace(/[^0-9]/g, ''))} onKeyDown={(e) => { if (e.key.length === 1 && !/[0-9]/.test(e.key)) e.preventDefault(); }} />
      </Field>
      <div className="grid grid-cols-2 gap-2">
        <Field id="m-fecha" label={t('movimientos.fecha')}>
          <input id="m-fecha" type="date" className="h-11 w-full rounded-lg border border-border bg-card px-3 text-body text-fg" value={dateKey} onChange={(e) => setDateKey(e.target.value)} />
        </Field>
        <Field id="m-nota" label={t('movimientos.nota')}>
          <input id="m-nota" className="h-11 w-full rounded-lg border border-border bg-card px-3 text-body text-fg placeholder:text-muted" placeholder={t('movimientos.nota.placeholder')} value={nota} onChange={(e) => setNota(e.target.value)} />
        </Field>
      </div>
      <div className="flex gap-2">
        <Button variant="ghost" className="flex-1" onClick={() => { setProductId(null); setQty('1'); setNota(''); setError(undefined); }}>
          <Eraser size={18} aria-hidden="true" />{t('movimientos.limpiar')}
        </Button>
        <Button className="flex-[2]" variant={kind === 'salida' ? 'danger' : 'primary'} onClick={() => void save()}>
