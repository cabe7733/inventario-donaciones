...
export function MedicalSupplyEntryModal({ supply, open, onClose }: { supply: MedicalSupply | null; open: boolean; onClose: () => void }) {
  ...
  return <Modal open={open} onClose={onClose} title={`Entrada de ${supply?.name ?? 'insumo'}`}><div className="flex flex-col gap-4"><div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><QuickPartySelect kind="donor" value={donorId} onChange={(id) => { setDonorId(id); if (id) void import('../../lib/donorOps').then(({ fetchParties }) => fetchParties('donor').then((parties) => setDonorName(parties.find((party) => party.id === id)?.full_name ?? null))); else setDonorName(null); }} /><Field id="supply-lot" label="Lote o código de fabricación"><input id="supply-lot" className={inputClass} value={lote} onChange={(e) => setLote(e.target.value)} /></Field><Field id="supply-expiry" label="Vencimiento"><DatePicker id="supply-expiry" value={expiry} onChange={setExpiry} /></Field><Field id="supply-qty" label="Cantidad" required><input id="supply-qty" type="number" min="0.01" step="any" className={inputClass} value={qty} onChange={(e) => setQty(e.target.value)} /></Field><Field id="supply-date" label="Fecha de entrada"><DatePicker id="supply-date" value={fecha} onChange={setFecha} /></Field><WarehouseSelect value={warehouseId} onChange={setWarehouseId} required /></div><div className="flex justify-end gap-2"><Button variant="ghost" onClick={onClose}>Cancelar</Button><Button onClick={() => void save()} disabled={saving}>{saving ? 'Guardando...' : 'Registrar entrada'}</Button></div></div></Modal>;
}
