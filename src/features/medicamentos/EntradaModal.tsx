...
export function EntradaModal({ medication, open, onClose }: { medication: Medication | null; open: boolean; onClose: () => void }) {
  ...
  return <Modal open={open} onClose={onClose} title={`Registrar entrada: ${medication?.name ?? 'medicamento'}`}><div className="flex flex-col gap-4"><div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><QuickPartySelect kind="donor" value={donorId} onChange={(id) => { setDonorId(id); if (id) void fetchParties('donor').then((parties) => setDonorName(parties.find((party) => party.id === id)?.full_name ?? null)); else setDonorName(null); }} /><Field id="med-lot" label="Código de fabricación / lote" required><input id="med-lot" className={inputClass} value={lote} onChange={(e) => setLote(e.target.value)} /></Field><Field id="med-expiry" label="Fecha de vencimiento" required><DatePicker id="med-expiry" value={expiry} onChange={setExpiry} min={todayKey()} /></Field><Field id="med-qty" label="Cantidad" required><input id="med-qty" type="number" min="1" step="any" className={inputClass} value={qty} onChange={(e) => setQty(e.target.value)} /></Field><Field id="med-date" label="Fecha de entrada"><DatePicker id="med-date" value={fecha} onChange={setFecha} /></Field></div><p className="text-caption text-muted">Los medicamentos tienen stock global y no se asocian a una bodega.</p><div className="flex justify-end gap-2"><Button variant="ghost" onClick={onClose}>Cancelar</Button><Button onClick={() => void save()} disabled={saving}>{saving ? 'Guardando...' : 'Registrar entrada'}</Button></div></div></Modal>;
}
